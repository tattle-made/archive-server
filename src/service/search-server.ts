...
export class SearchServer {
        ...
        return Axios.post('http://3.130.147.43:7000/find_duplicate', {
            ...
            .then((data) => {
                if (data.failed === 1) {
                    ...
                    const duplicate = data.result.filter(
                        (item: any) => item.dist === 0,
                    );
        ...
        return Axios.post('http://3.130.147.43:7000/find_duplicate', {
            ...
            .then((data) => {
                if (data.failed === 1) {
                    ...
                    const duplicateStories = data.result.filter(
                        (item: any) => item.source === 'story-scraper',
                    );
            ...
            .then((stories) => {
                ...
                return Promise.all(
                    stories.map((story: any) => get(story.doc_id)),
                );
            })
            .then((posts: any) => {
            ...
            .then((posts) => {
                ...
                return Promise.all(
                    posts.map((post: any) => getStoryByPostId(post.id)),
                );
            })
            .then((docs) => {
                return Promise.all(
                    docs.map((doc: any) => {
                        // tslint:disable-next-line:max-line-length
                        return Axios.get(
                            `http://${process.env.FCSEARCH_HOST}/metadataFromDoc?docId=${doc.docId}`,
                        ).then((res) => res.data);
                    }),
                );
            })
    ...
    public findTextWithinImage(text: string) {
        return Axios.post(
            `${process.env.SEARCH_HOST}:${process.env.SEARCH_PORT}/find_duplicate`,
            {
                text,
            },
        )
            ...
            .then((docs) => {
                return Promise.all(
                    docs.map((doc: any) => {
                        ...
                        console.log(doc);
                        return Axios.get(
                            `http://52.66.83.191:5001/api/metadata?docId=${doc.docId}`,
                        ).then((res) => res.data);
                    }),
                );
            })
    ...
    public searchTag(tag: string) {
        return (
            Axios.post('http://3.130.147.43:7000/search_tags', {
                tags: [tag],
            })
                .then((result) => result.data.docs.splice(0, 5))
                .then((docIds) =>
                    Promise.all(
                        docIds.map((docId: number) =>
                            get(docId).then((result) =>
                                appendMediaUrlToPost(result as Post),
                            ),
                        ),
                    ),
                )
